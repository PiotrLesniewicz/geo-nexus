...
class JobControllerIntegrationTest extends TestContainerConfig {
    ...
    private static final String JOB_IDENTIFIER = "JOB-2024-002"; // from test_data_job_leveling.sql
    ...
    void shouldReturnReports_afterProcessingTwoFiles() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/jobs/leveling")
                ...
                .andExpect(jsonPath("$[0].levelingType").value("ONE_WAY"))
                .andExpect(jsonPath("$[1].levelingType").value("ONE_WAY_DOUBLE"));
    }
    ...
    private MockMultipartFile buildMultipartFile(String resourcePath, String filename) {
        ...
        assertThat(stream).as("Test file not found: " + resourcePath).isNotNull();
        try (stream) {
            return new MockMultipartFile("file", filename, MediaType.TEXT_PLAIN_VALUE, stream);
        } catch (IOException e) {
            throw new RuntimeException("Failed to build multipart file from: " + resourcePath, e);
        }
    }
}